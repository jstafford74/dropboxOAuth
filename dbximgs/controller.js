...
module.exports.home = async (req, res, next) => {
    ...
    if (token) {
        try {
            let results = await getLinksAsync(token);
...
async function getLinksAsync(token) {
    ...
    let result = await listPathsAsync(token, '');
    ...
    temporaryLinkResults.forEach(
        (item) => {
            let newFile =
                new Dropfile(item.link, item.metadata.name);
            files.push(newFile);
        });
...
async function listPathsAsync(token, path) {
    ...
    try {
        ...
        var paths = result.entries.map(function (entry) {
            return entry.path_lower;
        });

        var names = result.entries.map(function (entry) {
            return entry.name
        });
